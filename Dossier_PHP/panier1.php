...
// Tableau des prix de chaque produit disponible
$prices = [
    'Kebab Veau' => 9,
    'Kebab Poulet' => 9,
    'Kebab Végétarien' => 9,
...
function calculateTotal(array $order, array $prices): float {
    ...
    // Ajoute le prix des accompagnements, boissons et extras
    foreach (['frites', 'drinks', 'extras'] as $category) {
    ...
    // Ajoute le supplément sauces pour les frites (la première gratuite, 0.20€ par sauce en plus)
...
if (isset($_GET['action']) && $_GET['action'] === 'recap') {
    ...
    if (empty($_SESSION['panier'])) {
        ...
        $totalPanier = 0;
        echo "<h3>Mon panier</h3>"; // Titre unique en haut
        foreach ($_SESSION['panier'] as $index => $item) {
            ...
            if (!empty($item['frites'])) {
                echo "<p><strong>Frites :</strong> " . implode(", ", array_map('htmlspecialchars', $item['frites'])) . "</p>";
            }
            if (!empty($item['sauces_frites'])) {
                echo "<p><strong>Sauces pour les frites :</strong> " . implode(", ", array_map('htmlspecialchars', $item['sauces_frites'])) . "</p>";
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    $frites = isset($_POST['frites']) && !empty($_POST['frites']) ? $_POST['frites'] : [];
    $saucesFrites = isset($_POST['sauces_frites']) && !empty($_POST['sauces_frites']) ? $_POST['sauces_frites'] : [];
    ...
    $order = [
        ...
        'frites' => is_array($frites) ? $frites : [$frites],
        'sauces_frites' => is_array($saucesFrites) ? $saucesFrites : [$saucesFrites],
    ...
    <div class="container">
        ...
        <form method="post" action="" id="orderForm" class="grand_section">
            ...
            <section class="section">
                <h2>1. Choisissez votre kebab</h2>
            ...
            <section class="section">
                ...
                <div class="drink-options">
                    <div class="drink-option" data-value="Coca cola" data-price="1.5">Coca cola (+1,5 €)</div>
                    ...
                    <div class="drink-option" data-value="Bierre" data-price="3">Bierre (+3 €)</div>
                </div>
            </section>
